...
<head>
    ...
    <title>Tambah Catatan Kesehatan — TernakDigital</title>
...
<div class="max-w-3xl mx-auto px-6 py-8">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <span>/</span>
        <span class="text-gray-700">Tambah Catatan</span>
    </div>

    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Tambah Catatan Kesehatan</h1>
        <p class="text-sm text-gray-500 mt-1">Catat kondisi dan tindakan kesehatan ternak</p>
    </div>
    ...
    <form action="{{ route('kesehatan.store') }}" method="POST"
        ...
        <div>
            ...
            <textarea id="tindakan" name="tindakan" rows="3"
                      placeholder="contoh: pemberian antibiotik, vaksin PMK, pemeriksaan rutin..."
                      class="w-full rounded-xl border @error('tindakan') border-red-400 @else border-gray-200 @enderror
                             px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 resize-none">{{ old('tindakan') }}</textarea>
            @error('tindakan')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
